...
scale=[185 290 425 620 885]; % yield different SNR
lag=[0]; % insert time lags between the signals in each hemisphere, or else random jitters applied
...
for ii=1:length(scale)
    ...
    Sim_Signal{ii}.trial{1}=Sim_Signal.trial{1}(:,1:1000)+data_base_avg.trial{1};
